...
1. Runway Picker - implementation of a boolean function which takes as inpu feedback from the runways and the incoming requests, and then outputs a suitable signal to be transmitted back.
...
4. Decoder - is a simple implementation of a 4:16 decoder to make the outputs easy to understand.
...
Inside the simulator, perss Ctrl+R to reset the simulation, and then Ctrl+K to start the clock used for the simulation.

Now requests for runways can be sent using the three input pins.
...
2. If the runway in required direction is not available, the other runway is given.
3. If both the runways are full then a wait signal is displayed.
